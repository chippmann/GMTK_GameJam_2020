...
onready var _parentState: State3D = _getParentState()
...
"""
func _assignStateMachine(node: Node) -> StateMachine3D:
	if node && !node.is_in_group(StateMachine3D.statemachineGroupName):
		return _assignStateMachine(node.get_parent())
	
	return node as StateMachine3D
